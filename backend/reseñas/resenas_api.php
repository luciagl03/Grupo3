...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        // Notificar al propietario de la plaza
        $stmtProp = $_conexion->prepare("SELECT p.DNI, p.Direccion FROM PLAZA p WHERE p.ID_plaza = ?");
        $stmtProp->bind_param('i', $id_plaza);
        $stmtProp->execute();
        ...
        if ($propResult) {
            ...
            crearNotificacion(
                ...
                'Has recibido una reseña de ' . $estrellas . ' en tu plaza de ' . ($propResult['Direccion'] ?? 'tu plaza') . '.',
                $id_plaza
            );
        }
