...
if ($es_propietario) {
    ...
    $backUrl       = 'reserva.php?id_plaza=' . $id_plaza;
}
?>
<!DOCTYPE html>
<html lang="es">
<head>
    ...
    <title>Contactar propietario — Zpot</title>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        .chat-layout { display:flex; flex-direction:column; height:100dvh; max-width:640px; margin:0 auto; background:#fff; box-shadow:var(--shadow-lg); }
        .chat-header { display:flex; align-items:center; gap:0.75rem; padding:0.875rem 1rem; border-bottom:1px solid var(--border); background:#fff; flex-shrink:0; position:sticky; top:0; z-index:10; }
        .chat-back { color:var(--brand-dark); text-decoration:none; display:flex; align-items:center; }
        .chat-avatar { width:38px; height:38px; border-radius:50%; background:var(--brand-yellow); display:flex; align-items:center; justify-content:center; font-weight:700; font-size:0.9rem; color:var(--brand-dark); flex-shrink:0; }
        ...
        .msg-bubble { padding:0.55rem 0.875rem; border-radius:18px; font-size:0.875rem; line-height:1.45; word-break:break-word; }
        ...
        .msg-fecha-sep { text-align:center; font-size:0.7rem; color:var(--text-muted); align-self:center; padding:0.3rem 0.75rem; margin:0.25rem 0; background:var(--brand-bg); border-radius:999px; }
        .chat-empty { flex:1; display:flex; flex-direction:column; align-items:center; justify-content:center; color:var(--text-muted); gap:0.5rem; padding:2rem; }
        .chat-empty p { font-size:0.875rem; text-align:center; margin:0; }
        .chat-input-wrap { display:flex; align-items:flex-end; gap:0.5rem; padding:0.75rem 1rem calc(0.75rem + env(safe-area-inset-bottom)); border-top:1px solid var(--border); background:#fff; flex-shrink:0; }
        .chat-textarea { flex:1; border:1.5px solid var(--border); border-radius:22px; padding:0.6rem 1rem; font-family:inherit; font-size:0.875rem; color:var(--brand-dark); resize:none; min-height:40px; max-height:120px; overflow-y:auto; line-height:1.4; transition:border-color 0.15s; }
        .chat-textarea:focus { outline:none; border-color:var(--brand-dark); }
        .chat-send { width:40px; height:40px; border-radius:50%; background:var(--brand-dark); border:none; cursor:pointer; display:flex; align-items:center; justify-content:center; flex-shrink:0; transition:background 0.15s, transform 0.1s; color:var(--brand-yellow); }
        .chat-send:hover { background:#1a1915; }
        .chat-send:active { transform:scale(0.92); }
        .chat-send:disabled { opacity:0.4; cursor:not-allowed; }
        .plaza-banner { background:var(--brand-bg); border-bottom:1px solid var(--border); padding:0.6rem 1rem; font-size:0.75rem; color:var(--text-muted); display:flex; align-items:center; gap:0.4rem; flex-shrink:0; }
    </style>
</head>
<body>
...
function enviarMensaje() {
    ...
    fetch(API, {
        ...
        body: JSON.stringify({ id_plaza: ID_PLAZA, dni_inquilino: DNI_INQUILINO, mensaje: texto })
    })
    .then(function(r){ return r.json(); })
    .then(function(data) {
        if (data.success) {
